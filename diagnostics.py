...
def get_average_kills(history_dict=history_dict):
    average_kills = avg(
        [run["kills"] for timestamp, run in history_dict.items()])  # calculate average kills from history dictionary
...
def get_death_probabilities(history_dict=history_dict):
    ...
    death_probabilities = {f"{world_id}-{level}-{loops}":
                               (sum([bool(
                                   run["world"] == world_id and run["level"] == str(level) and run["loops"] == str(
                                       loops)) for run in history_dict.values()]) / len(history_dict.values()))
                           for world_id in world_ids for level in [0, 1, 2, 3] for loops in
                           range(20)}  # (times_you_died_in_this_level / time_you_died_ever) for each level
    """
    TODO
